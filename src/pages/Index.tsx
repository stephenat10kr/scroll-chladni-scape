...
const Index = () => {
  const chladniRef = useRef<HTMLDivElement>(null);
  const [showScrollJack, setShowScrollJack] = useState(false);

  useEffect(() => {
    const handleScroll = () => {
      if (chladniRef.current) {
        const rect = chladniRef.current.getBoundingClientRect();
        setShowScrollJack(rect.top <= 0);
      }
    };
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="bg-blue-500 min-h-[200vh] w-full flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-xl md:text-2xl text-white max-w-2xl mx-auto">
            This section scrolls normally. The scroll-jacking effect will begin when you reach the next section.
          </p>
        </div>
      </div>

      {/* Reference div to detect when scrolljack should start */}
      <div ref={chladniRef} className="h-0 w-full" />

      {/* Chladni pattern with scrolljack sections */}
      <div className={`${showScrollJack ? "fixed top-0 left-0 w-full" : ""}`}>
        <ChladniPattern>
          <div className="flex flex-col">
      ...
      {/* Spacer to push content down when ScrollJack is fixed */}
      {showScrollJack && (
        <div style={{ height: "calc(100vh * 4)" }}>
          {/* This div takes up space of ScrollJack + Red box (3 sections + red box) */}
        </div>
      )}
    </div>
  );
